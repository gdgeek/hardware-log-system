...
export class LogService {
  ...
  async createLog(logData: LogInput): Promise<LogOutput> {
    ...
    // Verify signature (New Requirement)
    await SignatureService.verify(validatedData);
  ...
  private toLogOutput(log: Log): LogOutput {
    return {
      id: Number(log.id), // Ensure it's a number (Sequelize BIGINT comes back as string)
