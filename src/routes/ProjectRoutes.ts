 ...
 * 项目认证接口（公开接口）
 */
router.post('/auth', validateBody(projectAuthSchema), async (req: Request, res: Response) => {
  try {
 ...
 * 获取特定项目信息（需要管理员权限）
 */
router.get('/:id', adminAuthMiddleware, validateParams(projectIdSchema), async (req: Request, res: Response) => {
  try {
 ...
 * 创建新项目（需要管理员权限）
 */
router.post('/', adminAuthMiddleware, validateBody(createProjectSchema), async (req: Request, res: Response) => {
  try {
 ...
 * 更新项目信息（需要管理员权限）
 */
router.put('/:id', adminAuthMiddleware, validateParams(projectIdSchema), validateBody(createProjectSchema), async (req: Request, res: Response) => {
  try {
    const projectId = parseInt(req.params.id, 10);
    const projectData = req.body;
 ...
 * 删除项目（需要管理员权限）
 */
router.delete('/:id', adminAuthMiddleware, validateParams(projectIdSchema), async (req: Request, res: Response) => {
  try {
    const projectId = parseInt(req.params.id, 10);
 ...
 * 获取项目的列名映射（公开接口）
 */
router.get('/:id/column-mapping', validateParams(projectIdSchema), async (req: Request, res: Response) => {
  try {
    const projectId = parseInt(req.params.id, 10);
