...
function validateConfig(config: EnvConfig): void {
  ...
  if (config.dbPort < 1 || config.dbPort > 65535) {
    throw new Error(`DB_PORT must be between 1 and 65535, got: ${config.dbPort}`);
  }

  // Validate pool sizes
  if (config.dbPoolMin < 0) {
    throw new Error(`DB_POOL_MIN must be non-negative, got: ${config.dbPoolMin}`);
  }
  if (config.dbPoolMax < config.dbPoolMin) {
    throw new Error(`DB_POOL_MAX must be >= DB_POOL_MIN, got: ${config.dbPoolMax} < ${config.dbPoolMin}`);
  }

  // Validate page sizes
  if (config.defaultPageSize < 1) {
    throw new Error(`DEFAULT_PAGE_SIZE must be positive, got: ${config.defaultPageSize}`);
  }
  if (config.maxPageSize < config.defaultPageSize) {
    throw new Error(`MAX_PAGE_SIZE must be >= DEFAULT_PAGE_SIZE, got: ${config.maxPageSize} < ${config.defaultPageSize}`);
  }
